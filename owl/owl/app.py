...
def run_script(script_dropdown, question, progress=gr.Progress()):
    ...
    if answer_text and status_message(current_process) == "✅ 执行成功":
        try:
            ...
            # 如果没有提取到分析内容，使用整个答案文本
            if not analysis_text.strip():
                analysis_text = answer_text
                
            # 包装成完整报告
            full_report = answer_text
...
def extract_chat_history(logs):
    ...
    try:
        ...
        # 提取问题（更精确的匹配）
        question_match = re.search(r"(问题:|Questions:)\s*(.*?)(?=\n|$)", logs_text, re.DOTALL | re.IGNORECASE)
        if question_match:
            question = question_match.group(2).strip()
            
        # 提取答案（更精确的匹配，包括多行答案）
        answer_match = re.search(r"Answer:\s*(.*?)(?=\nLog:|Module:|✅|$)", logs_text, re.DOTALL | re.IGNORECASE)
        if answer_match:
        ...
        if chat_match:
            ...
            try:
                ...
                print("JSON解析错误，尝试正则提取聊天历史...")
                # 备用正则提取，以防JSON解析失败
                role_content_pattern = r"Role:\s*(.*?)\nContent:\s*(.*?)(?=\nRole:|$)"
                matches = re.findall(role_content_pattern, chat_json_str, re.DOTALL | re.IGNORECASE)
                for role, content in matches:
                    chat_history.append((role.strip(), content.strip()))
        else:
            # 如果没有Chat History JSON，则使用问题和答案构建基本聊天历史
            if question:
...
def load_latest_result():
    ...
    # 排除聊天历史文件
    result_files = [f for f in result_files if not f.name.startswith("chat_history_")]
    ...
    try:
        ...
        answer_match = re.search(r"## 回答摘要\s*\n\n(.*?)\n\n", content, re.DOTALL)
...
def load_latest_chat_history():
    ...
    # 查找所有聊天历史文件
    chat_files = list(results_dir.glob("chat_history_*.md"))
...
def create_ui():
    ...
    # 尝试加载历史结果
    last_question, last_answer = load_latest_result()
    last_chat_history = load_latest_chat_history()
            ...
            with gr.TabItem("会话"):
                with gr.Row():
                    ...
                    with gr.Column(scale=5):
                        ...
                        question_input = gr.Textbox(
                            ...
                            label="问题",
                            value=last_question if last_question else "",
                        )
                ...
                with gr.Tabs() as result_tabs:
                    ...
                    with gr.TabItem("聊天历史"):
                        chat_output = gr.JSON(
                            label="聊天交互记录", 
                            value=last_chat_history if last_chat_history else None
                        )
